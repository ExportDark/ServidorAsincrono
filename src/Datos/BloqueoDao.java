...
public class BloqueoDao {
    ...
    private static final String SQL_DELETE = "DELETE FROM Bloqueo WHERE pedro = ? AND pablo = ?";
    List<Bloqueo> bloqueos;

    public boolean comuniacionBloqueada(Bloqueo bloqueo) {
        for (Bloqueo x : listar()) {
            if ((bloqueo.getPedro().equals(x.getPedro()) && bloqueo.getPablo().equals(x.getPablo())) || (bloqueo.getPedro().equals(x.getPablo()) && bloqueo.getPablo().equals(x.getPedro()))) {
                return true;
            }
        }
        return false;
    ...
    public List<Bloqueo> listarBloqueados(Bloqueo bloqueo) {
        bloqueos = listar();
        for (Bloqueo b : bloqueos){
            if (b.getPedro().equals(bloqueo.getPedro())) {
                bloqueos.add(b);
            }
        }
        return bloqueos;
    ...
    public int desbloquear(Bloqueo bloqueo) {
        int registros = 0;
